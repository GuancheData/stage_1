...
class CrawlerController:
    def __init__(self, datalake_path, logs_Path, total_books = 1000, datalake_structure="date"):
        self.requester = GutenbergRequest()
        self.storage = BookStorageId(Path(datalake_path)) if datalake_structure == "id" else BookStorageDate(
            Path(datalake_path))
