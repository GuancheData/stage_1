...
OUTPUT_PATH = "HIERARCHICAL_INDEX"
...
class HierarchicalQueryPerformanceTest(unittest.TestCase):
    ...
    def search_hierarchical(self, query_term):
        ...
        term_path = Path(OUTPUT_PATH)
        for char in query_term:
            term_path = term_path / char
        term_path = term_path / f"{query_term}.txt"
    ...
    def test_query_performance(self):
        ...
        for query in TEST_QUERIES:
            ...
            for _ in range(NUM_QUERY_RUNS):
                results = self.search_hierarchical(query)
        ...
        HierarchicalQueryPerformanceTest.results = {
    ...
    def tearDownClass(cls):
        if cls.results:
            print(f"\n{'=' * 70}")
            print(f"HIERARCHICAL FOLDER STRUCTURE - SUMMARY")
            print(f"{'=' * 70}")
